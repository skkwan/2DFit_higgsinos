...
def plot_toy_fit(toys, model, result, met, mll, mass_point,
    ...
    """
    Plot 1D projections of a toy dataset with the best-fit model overlaid,
...
if __name__ == "__main__" :
    ...
    file_results = ROOT.TFile(f"/eos/cms/store/group/phys_susy/skkwan/toys/toy_results_{m1}_{m2}_nExp_{n_experiments}_nsig_{n_sig_in}_nbkg_{n_bkg_in}.root", "RECREATE")
    for i in range(n_experiments):
        mcs.fitResult(i).Write(f"fitResult_{i}")
    file_results.Close()

    file_data = ROOT.TFile(f"/eos/cms/store/group/phys_susy/skkwan/toys/dump_data_{m1}_{m2}_nExp_{n_experiments}_nsig_{n_sig_in}_nbkg_{n_bkg_in}.root", "RECREATE")
    for i in range(n_experiments):
        mcs.genData(i).Write(f"genData_{i}")
    file_data.Close()
    print(f"Saved {n_experiments} generated datasets to /eos/cms/store/group/phys_susy/skkwan/toys/*dump_data.root")
